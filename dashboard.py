...
# ---------------------------------------------------------------------------
# Load model directly in-process -- no FastAPI/Render needed for this
# deployment. A separate api.py still exists in this repo and demonstrates
# the same model exposed as a standalone service (see README).
# ---------------------------------------------------------------------------
@st.cache_resource
def load_model():
...
def run_inference(image: Image.Image):
    ...
    return {
        ...
        "heatmap_image": Image.fromarray(overlay_rgb),
    }
...
.stApp { background: #0A0D10; color: #D8DEE4; }
#MainMenu, header, footer { visibility: hidden; }

.scan-header {
    display: flex; align-items: baseline; gap: 14px;
    padding: 28px 0 8px 0; border-bottom: 1px solid #232A31; margin-bottom: 28px;
}
.scan-header .mark { font-family: 'JetBrains Mono', monospace; color: #F2A93B; font-size: 22px; }
.scan-header h1 { font-size: 22px; font-weight: 700; letter-spacing: 0.02em; color: #EDEFF2; margin: 0; }
.scan-header .sub { font-family: 'JetBrains Mono', monospace; font-size: 12px; color: #5A6672; letter-spacing: 0.08em; text-transform: uppercase; }

.panel-label { font-family: 'JetBrains Mono', monospace; font-size: 11px; letter-spacing: 0.12em; text-transform: uppercase; color: #5A6672; margin-bottom: 10px; }

.scan-bed { position: relative; background: #10151A; border: 1px solid #232A31; border-radius: 2px; padding: 18px; }
.scan-bed::before, .scan-bed::after, .scan-bed .br1, .scan-bed .br2 {
    content: ''; position: absolute; width: 18px; height: 18px; border-color: #F2A93B; border-style: solid; opacity: 0.85;
}
...
.stamp {
    font-family: 'JetBrains Mono', monospace; font-weight: 700; font-size: 28px; letter-spacing: 0.15em;
    padding: 14px 34px; border: 3px solid currentColor; border-radius: 4px; transform: rotate(-3deg); display: inline-block;
}
...
.metric-row { font-family: 'JetBrains Mono', monospace; display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #1B2126; font-size: 13px; }
...
.stButton>button {
    background: #F2A93B; color: #0A0D10; font-family: 'JetBrains Mono', monospace; font-weight: 700;
    letter-spacing: 0.08em; text-transform: uppercase; border: none; border-radius: 2px; padding: 10px 22px; width: 100%;
}
.stButton>button:hover { background: #FFC266; }

[data-testid="stFileUploader"] { background: #10151A; border: 1px dashed #2A323A; border-radius: 2px; padding: 10px; }
</style>
...
with col_result:
    ...
    if uploaded_file is not None and run:
        ...
        stamp_class = "fail" if result["pred_label"] == "defective" else "pass"
        stamp_text = "DEFECT" if result["pred_label"] == "defective" else "PASS"

        st.markdown(f"""
        <div class="stamp-wrap"><span class="stamp {stamp_class}">{stamp_text}</span></div>
        <div class="metric-row"><span class="k">Anomaly Score</span><span class="v">{result['pred_score']:.4f}</span></div>
